...
$(function() {
    /** 创建数据表格	 */
    xjdgl_dataGrid = $('#dataGrid_report_zerenren').datagrid({
        url : path+'/ReportByZerenr',
        striped : true,
        rownumbers : true,
        frozenColumns:[[
            {width: '100',title: '责任人',field: 'ZERENREN',align:'center'},
            {width: '200',title: '不合格记录(次)',field: 'ZERENREN_COUNT',
            },
        ]],
    });
    $('#searchForm_ZeRenRen').show();
    ////初始化查询条件
...
/** 表单查询 */
function searchForm_ZeRenRenFun() {
    var prjName = $("#projectid_query_id").combobox('getText');
    $('#proiectname_query').val(prjName);
    xjdgl_dataGrid.datagrid('load', $.serializeObject($('#searchForm_ZeRenRen')));
}

/*
 * 重置
 * */
function cleandZeRenRenJLFun() {
    $('#searchForm_ZeRenRen input').val('');
