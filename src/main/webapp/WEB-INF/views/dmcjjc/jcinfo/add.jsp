...
$(function() {
	var isno=${isno};
	var absentDetails=${absentDetails};
	var success=${success};
	var failure=${failure};
	if(judge(isno)||judge(absentDetails)){
		$.messager.alert({
			title:'提示',
			msg:'导入成功的记录数:'+success+'<br/>导入失败的记录数:'+failure+'<br/>不存在的监测点:<br/>'+isno+'<br/>不是该项目的监测点:<br/>'+absentDetails,
			height:'180px'
		...
		$.messager.alert({
			title:'提示',
			msg:'导入成功的记录数:'+success+'<br/>导入失败的记录数:'+failure,
			height:'180px'
		});
	}
        ...
        $('#addForm').form({
        		url : '${path}/dmcjjc/dmcjjcinfo/save',
            	onSubmit : function() {
                progressLoad();
    ...
    function btnConfirm(){
        ...
        $.ajax({
            ...
            data = $.parseJSON(data);
	            	...
	            	//计算后的内容在明细表格中新增一行
	            	if($('#flag').val() == "insert"){
	            		addRow(data,$('#detailRremarks').combobox('getValue'));
	            	}else if($('#flag').val() == "update"){
	            		modiRow(data,$('#detailRremarks').combobox('getValue'));
	            	}
	   			 }else{
	   				 //没有项目信息，不能保存该点
    ...
    function saveFun(){
    	...
    	var rows = $("#dataGrid").datagrid("getRows"); 
    	if(rows.length <=0 ){
    		parent.$.messager.alert('提示', '无日常监测明细，新增失败。', 'info');
    		return;
    	}
    	if (editRow != undefined) {
            $("#dataGrid").datagrid('endEdit', editRow);
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div class="easyui-panel" title="监测信息" data-options="region:'north',border:false,collapsible:true"  
		...
		<form id="addForm" method="post">
		     <input id="dataSta" value='modified' type="hidden">
			...
			<table class="grid">
				<tr>
					...
					<td colspan="3" style="width:200px" >
						<div style="display: none;">
							<select id="gcbh" name="gcbh" class="easyui-combobox"
								data-options="required:true" style="width: 140px;"></select>
						</div> 
						<span id='projname'></span>
					</td>
					<td class="form-td-label" style="width: 100px">备&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;注：</td>
					<td style="width:230px" rowspan="3"><input id="remarks" name="remarks" class="easyui-textbox"
						data-options="validType:'length[1,200]',required:false,prompt:'请输入备注',multiline:true"
						style="width: 100%; height: 100%"></td>
				</tr>
				<tr>
					<td class="form-td-label">本次监测时间：</td>
					<td><input editable="false" id="jcTime" name="jcTime"
						class="easyui-datetimebox"
						data-options="required:true,prompt:'本次监测时间'" value="${jcTime}"
						style="width: 100%;" /></td>
					<td class="form-td-label"><span style="color: red;">*</span>天&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;气：</td>
					<td><select editable="false" id="tianqi" name="tianqi"
				...
				<tr>
					...
					<td><a href='${path }/dmcjjc/dmcjjcinfo/download?filepath=${fileUrl}'>${fielName}</a></td>
	...
	<div class="easyui-panel" data-options="region:'center',border:false" style="width:100%;height:100px;">
		...
		<a onclick="openWin('insert',0);" href="javascript:void(0);"
			...
			data-options="plain:true,iconCls:'icon-add'">新增明细</a> <a
			onclick="editDetailFun();" href="javascript:void(0);"
			class="easyui-linkbutton"
			data-options="plain:true,iconCls:'icon-edit'">编辑明细</a> <a
...
<!-- 新增明细时弹出的窗口 -->
<div id="win" class="easyui-window" title="日常监测数据"
