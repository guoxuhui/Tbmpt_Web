...
var editRow = undefined;
    $(function() {
        $('#addForm').form({
        		url : '${path}/dmcjjc/dmcjjcinfo/updateAll',
            	onSubmit : function() {
            		 progressLoad();
    ...
    function btnConfirm(){
        ...
        $.ajax({
            ...
            data = $.parseJSON(data);
	            	...
	            	//计算后的内容在明细表格中新增一行
	            	if($('#flag').val() == "insert"){
	            		addRow1(data,detailRremarks);
	            	}else if($('#flag').val() == "update"){
	            		modiRow1(data,detailRremarks);
	            	}
	   			 }else{
	   				 //没有项目信息，不能保存该点
    ...
    function addRow1(obj,detailRremarks){
    	$.ajax({
  ...
  //修改的时候要检查点位是不是该线路下以及是否在监测信息中重复存在
    function modiRow1(obj,detailRremarks){
    	 $.ajax({
     		url: "${path}/dmcjjc/dmcjjcinfo/checkPointInLine",
     		data: {
     			jcdno: $('#jcdbh').val(),
...
<div class="easyui-layout" data-options="fit:true,border:false">
	...
	<form id="addForm" method="post">
		...
		<input id="impFilePath" name="impFilePath" type="hidden"  value="${jc.impFilePath}">
