...
<body>
	...
	<div class="container" id="container">
		<div class="page input js_show">
			...
			<div class="page__bd">
				<form id="form" method="post" action="${staticPath}/open/aqyh/editWorklogDo">
				
              		<input name="id" type="hidden" value="${log.id}"/>
         			<input name="createUser" type="hidden" value="${log.createUser}"/>
         			<input name="createUsername" type="hidden" value="${log.createUsername}"/>
         			<input name="createOrgz" type="hidden" value="${log.createOrgz}"/>
         			<input name="createOrgzname" type="hidden" value="${log.createOrgzname}"/>
         			<input name="createTime" type="hidden" value="${log.createTime}"/>
         			<input name="updateUser" type="hidden" value="${log.updateUser}"/>
         			<input name="updateUsername" type="hidden" value="${log.updateUsername}"/>
         			<input name="updateOrgz" type="hidden" value="${log.updateOrgz}"/>
         			<input name="updateOrgzname" type="hidden" value="${log.updateOrgzname}"/>
         			<input name="updateTime" type="hidden" value="${log.updateTime}"/>
					...
					<div class="weui-cells weui-cells_form">
						<div class="weui-cell">
                            ...
                            <div class="weui-cell__bd">
                                <input name="workDay" class="weui-input" type="date" placeholder="日志日期" required value="${log.workDay}"/>
                            </div>
                        </div>
						<div class="weui-cell">
							<div class="weui-cell__bd">
								<textarea name="content" class="weui-textarea" placeholder="请输入安全日志，内容不超过500个字！" rows="5" required>${log.content}</textarea>
